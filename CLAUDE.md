...
- **Week 5** (accepted) — Entity extraction (DeepMind Principle 4): a pluggable `EntityExtractor` seam (`duplex-bridge/src/duplex_bridge/entities/`) with a local default (`LocalQwenVLExtractor`, Qwen3-VL-4B-4bit via MLX) and a `GeminiFlashLiteExtractor` fallback over the same seam. Emits typed entities from cursor tiles and routes them (place→Maps, date/todo→Calendar, code_span→IDE) off the hot path, before the audio turn (`server._maybe_schedule_entities`). Measured vision-only on 31 real tiles: **71% type-recall, 57% value-recall, 65% precision, p50 ~3.9 s** (4B vs 2B's 36% type-recall). Tiles clamped to 384 px to stay under the macOS GPU watchdog (`ENTITY_VLM_TILE_MAXPX`; the uncapped 512 px prefill kernel trips `kIOGPUCommandBufferCallbackErrorHang` at only ~3.8 GB — a timeout, not OOM). See `docs/week5-entity-acceptance.md`.
- **Week 6** (accepted) — Async background worker (`duplex-bridge/src/duplex_bridge/worker.py`): `BackgroundWorker.submit` runs long tool calls off the audio hot path (coroutines as loop tasks, blocking calls in a thread pool) and returns immediately; `ToolDispatcher` registers as the session's `on_tool_call` (wired in `__main__`, handlers land in Week 7). Preserves the `DuplexSession` seam. Proven under load (`scripts/bench/bench_worker_hotpath.py`): a 200 ms audio tick holds **max 2.3 ms** lateness through 20×0.3 s of tool work, vs **6142 ms** stall when the same work runs inline. See `docs/week6-async-worker-acceptance.md`.
- **Week 7** (accepted) — Host app actions (`duplex-bridge/src/duplex_bridge/actions/`): `compare_products` fetches each product's Wikipedia summary, renders a side-by-side page, and opens it in real Chromium (Playwright — `navigate` seam is computer-use-swappable); `rewrite_function_async` applies a verified async rewrite to a real file (model-provided source or a deterministic `ast` transform). Declared as Gemini tools (`TOOL_DECLARATIONS` → `GeminiLiveSession(tools=...)`), dispatched off the hot path via the Week-6 worker; `DuplexSession` seam untouched. Verified end-to-end headlessly (simulated tool call → real Chrome + real file edit; 10 tests; `scripts/demo/demo_host_actions.py`). The live speech→tool-call hop is user-driven (`GEMINI_API_KEY` + mic). See `docs/week7-host-actions-acceptance.md`. **Plus a general cross-application `computer_use(goal)` tool** (`actions/computer.py`): a `Computer` seam (OS-level screenshot + mouse + keyboard; macOS via Quartz `CGEvent` + `screencapture`) driven by a `ComputerUseExecutor` perceive→decide→act loop with a pluggable vision `Policy`. The bespoke tools are fast paths; `computer_use` is the general fallback. Executor loop verified deterministically (`test_computer.py`, 7 tests); the live desktop drive + real policy are user-run (Accessibility perm). See `docs/week7b-computer-use.md`.
- **Week 8** (accepted) — FD-bench-style eval (`scripts/bench/eval_fd.py`): drives the real `MicCapture` turn machine + `SpeakerOutput`/`GeminiLiveSession` barge-in with synthetic audio/events, scoring the full-duplex behaviors **5/5** (backchannel rejected via the onset debounce, talk-over opens a turn, turn-taking closes on silence, barge-in flushes playback, interrupt flag detected) + the custom pointer-deixis suite at **87.1%** (Week 4). Deterministic, gated in the suite (`duplex-bridge/tests/test_fd.py`). See `docs/week8-fd-eval-acceptance.md`.
- **Week 9** — Decoupled deixis + delegated action stack. (a) **Tool results flow back**: `ToolDispatcher` keeps `fc.id`, finished jobs return as `FunctionResponse` (WHEN_IDLE — completions never barge into speech); long tools are `NON_BLOCKING` with an immediate SILENT "started" ack (probe-validated: `scripts/diag/probe_tool_response_scheduling.py`); server `tool_call_cancellation` cancels in-flight jobs. (b) **Deixis resolve-on-settle** (`duplex_bridge/deixis/`): a Flash-Lite `PointerReferentResolver` reads tile+AX off the hot path each time the cursor settles on a new target; the latest referent injects as the `pointer=` annotation, the turn's settle history feeds delegated tasks (`pointer_history_for_delegate`). (c) **DelegateAgent** (`actions/delegate.py`): the live model emits one `delegate_task(goal)`; a gemini-3.5-flash Interactions loop orchestrates `run_shell`/`run_applescript`/`browser_*` (persistent per-task Chromium, `actions/browser.py`)/nested `computer_use` (Week-7b executor + the real `GeminiComputerUsePolicy`, `actions/computer_policy.py`). (d) **TaskManager** (`actions/tasks.py`): N concurrent named tasks, desktop mutex (one mouse driver at a time), `check_tasks`/`confirm_task`. (e) **Safety** (`actions/safety.py`): allowlisted commands run autonomously; everything else pauses for voice confirmation; destructive patterns always confirm. See `docs/week9-delegate-acceptance.md`.
- Post-Week-9 — Windows (UI Automation) + Linux (AT-SPI) portability; `DuplexSession` adapter for TML swap; combined Interactions loop (custom tools + built-in computer_use in one chain — probe already PASSes)
